<template>
  ...
  <!--资产统一监控看板-->
      <div class="lg:flex md:flex" style="min-height: 340px;margin-top: 10px;margin-left: 10px">
        <!--  分两个tab,一个是含3d模型的tab,一个是不含3d模型的tab  -->
        <div class="lg:w-7/10 w-full !mr-4 enter-y" style="background-color: #fff">
          <AssetRealtimeLine :entityId="assetId" class="enter-y"/>
        </div>
        <div class="lg:w-3/10 w-full !mr-4 enter-y" style="background-color: #fff">
          <LabelEntityPieMetric :entityId="assetId" entityType="ASSET" class="enter-y"/>
        </div>
      </div>

    <div class="lg:flex md:flex" style="min-height: 310px; max-height:340px; margin-top: 10px;margin-left: 10px">
      <div class="lg:w-4/10 w-full !mr-4 enter-y" style="background-color: #fff">
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(){
      ...
      // 此处可以得到用户ID
      const assetId = ref(route.params?.id).value as string;
      return {assetId}
    }
  });
</script>
